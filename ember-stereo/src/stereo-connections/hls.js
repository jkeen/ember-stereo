...
export default class HLSSound extends BaseSound {
  ...
  _setupHLSEvents(instance, HLS) {
    ...
    instance.on(HLS.Events.FRAG_CHANGED, (e, f) => {
      this.skippedFragments = 0; // a fragment played cleanly; reset the skip budget
  ...
  // Avoid player crash on empty/corrupt fragments (fatal fragParsingError).
  // recoverMediaError() loops on the same bad fragment, so we seek past
  // it to trigger a brief skip instead of killing the player.
  ...
  // video.currentTime throws on a non-finite value (the secondary NaN crash
  // once media detaches). Guard every write through here.
  ...
  // A lazy chunk, so the first sound that needs it waits on the download before
  // any audio. An app expecting HLS can warm it with
  // `stereo.prewarmConnection('HLS')`.
  static preload() {
