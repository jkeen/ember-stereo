...
export default class HLSSound extends BaseSound {
  ...
  // recoverMediaError() loops on the same bad fragment (fatal fragParsingError), so seek past it instead of killing the player.
  ...
  // video.currentTime throws on non-finite values (NaN once media detaches), so route every write through here.
  ...
  pause() {
    ...
    // Only a live playlist pulls fragments forever; halting an archive's loader would force rebuffering from scratch on resume.
    if (this.live) {
      this.hls.stopLoad();
