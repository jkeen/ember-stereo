...
export default class Stereo extends Service.extend(EmberEvented) {
  ...
  */
  ...
  // Resolve a connection class by key, whether the app registered it or the
  // service owns it.
  _findConnection(connectionKey) {
    ...
    // The casting connections are service-built and force-injected, never
    // registered, but Chromecast has the heaviest thing to warm.
  ...
  _loadOutlet(identifier) {
    ...
    // an unset src: just set it (first cast, before any prewarmCast landed).
  ...
  _onCastTargetChange(isWireless) {
    ...
    // unwarmed url) makes Safari drop then re-establish the AirPlay route,
