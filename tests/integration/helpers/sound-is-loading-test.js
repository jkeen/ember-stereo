...
module('Integration | Helper | sound-is-loading', function (hooks) {
  ...
  test('it renders loading status', async function (assert) {
    ...
    assert.equal(this.element.textContent.trim(), 'sound-is-loading', 'helper reports loading when loading');
  ...
  test('it renders loading status when url is a function', async function (assert) {
    ...
    assert.equal(this.element.textContent.trim(), 'sound-is-loading', 'helper reports loading');

    await waitUntil(() => {
      return this.element.textContent.trim() == 'is-not-loading'
    }, { timeout: 5000 });
    assert.equal(this.element.textContent.trim(), 'is-not-loading', 'helper reports not loading when finished');
  });
  ...
  test('it renders loading status when url is an array', async function (assert) {
    ...
    this.set('url', ['/good/200/loading-array.mp3', '/good/1000/loading-arry-2.mp3'])
