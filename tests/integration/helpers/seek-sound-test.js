...
module('Integration | Helper | seek-sound', function(hooks) {
  ...
  test('it seeks to seconds', async function (assert) {
    ...
    await render(
      hbs`<button type="button" {{on 'click' (seek-sound this.url 1 unit='seconds')}}>seek</button>`
    );
    await click('button');
  ...
  test('it seeks to percentage', async function (assert) {
    ...
    assert.equal(sound.position, 6000, 'position is 6000');
    await render(
      hbs`<button type="button" {{on 'click' (seek-sound this.url 0 unit='percentage')}}>seek</button>`
    );
    await click('button');
