...
module('Integration | Helper | fast-forward-sound', function(hooks) {
  ...
  test('it fast forwards', async function(assert) {
    ...
    await render (hbs`<button type="button" {{on 'click' (fast-forward-sound this.url)}}>fast forward</button>`);
    await click('button');
  ...
  test('it fast forwards in custom increment', async function(assert) {
    ...
    assert.equal(sound.position, 0, 'position is zero');
    await render(
      hbs`<button type="button" {{on 'click'(fast-forward-sound this.url increment=1500)}}>fast forward</button>`
    );
    await click('button');
