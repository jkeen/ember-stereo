...
class RemotePosition {
  ...
  estimate(isPlaying) {
    return isPlaying ? this._anchorMs + (Date.now() - this._anchorWall) : this._anchorMs;
  }
  positionFor({
    reportedMs = null,
    isPlaying = false,
    ...
    if (this.isSettling) {
      let caughtUp = reportedMs != null && Math.abs(reportedMs - this._settleTarget) <= this.toleranceMs;
      if (!caughtUp) {
        return this.estimate(isPlaying);
      }
      this._settleTarget = null;
...
export { RemotePosition as default };
//# sourceMappingURL=remote-position.js.map
