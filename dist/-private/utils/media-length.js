 ...
 * @class MediaLength
 */
let MediaLength = (_class = class MediaLength {
  constructor({
    sampleMs = 250
  } = {}) {
    _initializerDefineProperty(this, "_samples", _descriptor, this);
    _initializerDefineProperty(this, "_provedLive", _descriptor2, this);
    // Sampling is how it learns, so it lives with the thing being learned.
    _defineProperty(this, "watchTask", buildTask(() => ({
      context: this,
      generator: function* ({
        durationMs,
        isPlaying,
  ...
  record(durationMs, timeMs = Date.now()) {
    ...
    this._samples = [...this._samples.slice(-19), {
      durationMs,
      timeMs
    }];
  ...
  estimate({
    ...
    if (elementDurationMs > IMPLAUSIBLE_DURATION_MS || this.isLive) {
      let recorded = Number.isFinite(elementDurationMs) ? null : this.seekableMs(seekable) ?? this._lastRecordedMs;
      return recorded ?? Infinity;
    }
    return elementDurationMs;
  }
}, _descriptor = _applyDecoratedDescriptor(_class.prototype, "_samples", [tracked], {
  configurable: true,
  enumerable: true,
  writable: true,
...
}), _descriptor2 = _applyDecoratedDescriptor(_class.prototype, "_provedLive", [tracked], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: function () {
...
export { MediaLength as default, durationGrowsWithTheClock };
//# sourceMappingURL=media-length.js.map
