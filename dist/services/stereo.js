...
import { race, waitForEvent, didCancel } from 'ember-concurrency';
...
import CastCoordinator from '../-private/casting/coordinator.js';
import SoundIdentityMap from '../-private/utils/sound-identity-map.js';
import Strategizer from '../-private/utils/strategizer.js';
import Sound from '../-private/utils/sound.js';
...
import { EVENT_MAP } from '../-private/event-map.js';
export { SERVICE_EVENT_MAP } from '../-private/event-map.js';

var _class, _descriptor, _descriptor2, _descriptor3, _descriptor4, _descriptor5, _Stereo;
...
let Stereo = (_class = (_Stereo = class Stereo extends Service.extend(EmberEvented) {
  constructor() {
    ...
    _defineProperty(this, "_identityMap", new SoundIdentityMap());
    // Tracked, unlike the identity map, since loads happen outside render.
    _defineProperty(this, "_sounds", new TrackedSet());
    _defineProperty(this, "_previewSounds", new WeakSet());
    _initializerDefineProperty(this, "_currentSound", _descriptor2, this);
    /**
     ...
     * @public
     */
    _initializerDefineProperty(this, "_playbackSpeed", _descriptor4, this);
    /**
     ...
     * @public
     */
    _initializerDefineProperty(this, "isMobileDevice", _descriptor5, this);
    /**
    ...
    _defineProperty(this, "loadTask", buildTask(() => ({
      ...
      generator: function* (urlsOrPromise, _options) {
        ...
        let urlsToTry = yield sound.resolveUrls();
        ...
        let connection = yield sound.load(_options);
        if (connection) {
    ...
    _defineProperty(this, "playTask", buildTask(() => ({
      ...
      generator: function* (urlsOrPromise, options = {}) {
        ...
        if (previouslyPlayingSound && previouslyPlayingSound === this.findSound(urlsOrPromise)) {
          return {
    ...
    _defineProperty(this, "resolveIdentifierTask", buildTask(() => ({
      ...
      generator: function* (identifier) {
        return (yield this.findSound(identifier)?.resolveUrls()) ?? [];
      }
    }), {
      maxConcurrency: 5
  ...
  _buildStrategies(urlsToTry, options) {
    ...
    if (this.cast.shouldCastUrl(options.castUrl)) {
      ...
      return [this.cast.strategyFor(options.castUrl, options.metadata, {
  ...
  _handleLoadError({
    ...
    let erroredSound = sound ?? this.findSound(makeArray(failures)[0]?.url ?? null);
    this.trigger('audio-load-error', {
      sound: erroredSound,
      failures: failures,
      error: errorMessage
  ...
  findSound(identifier) {
    ...
    let sound = new Sound(identifier, {
      owner: getOwner(this)
    });
  ...
  _relayEvent(eventName, info = {}) {
    next(() => {
      ...
      debug(eventName === 'audio-position-changed' ? 'ember-stereo:position' : 'ember-stereo:service')(eventName, info);
    });
  }
  ...
  _mediaSessionActionsFor(sound) {
    let registered = makeArray(sound?.urls ?? sound?.url).map(url => this._mediaSessionActions.get(normalizeIdentifier(url))).find(Boolean);
    return registered ?? {};
  }
  _clearNowPlaying() {
  ...
  _updatePositionState(sound) {
    if (!sound || typeof navigator === 'undefined' || !('mediaSession' in navigator) || typeof navigator.mediaSession.setPositionState !== 'function') {
      return;
    }
...
}), _descriptor2 = _applyDecoratedDescriptor(_class.prototype, "_currentSound", [tracked], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: function () {
...
}), _descriptor3 = _applyDecoratedDescriptor(_class.prototype, "_volume", [tracked], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: function () {
...
}), _descriptor4 = _applyDecoratedDescriptor(_class.prototype, "_playbackSpeed", [tracked], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: function () {
...
}), _descriptor5 = _applyDecoratedDescriptor(_class.prototype, "isMobileDevice", [tracked], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: function () {
