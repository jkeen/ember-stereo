...
import RemotePosition from '../-private/casting/remote-position.js';
import { getMimeType } from '../-private/utils/mime-types.js';
import { loadCastSdk } from '../-private/casting/google-cast-sdk-loader.js';
 ...
 * @extends BaseSound
 */
class Chromecast extends Sound {
  static toString() {
    return 'Chromecast';
  }
  static canPlay() {
  ...
  setup() {
    ...
    if (player?.mediaInfo && isSameAudio(player.mediaInfo.contentId, this.url, {
      exact: true
    })) {
      this._onLoaded();
      // Announcing the play starts the position poll, and the Sound's connection setter re-announces it once assigned.
      if (player.playerState === window.chrome?.cast?.media?.PlayerState?.PLAYING) {
  ...
  _loadMedia(session) {
    ...
    let contentType = this.options?.contentType || getMimeType(this.url) || 'audio/mpeg';
    ...
    session.loadMedia(request).then(() => this._onLoaded(), error => this.trigger('audio-load-error', {
      sound: this,
  ...
  _sendPlaybackRate() {
    ...
    Promise.resolve(session.sendMessage(CAST_MEDIA_NAMESPACE, {
  ...
  play() {
    ...
    if (this.isStream && state !== states.PLAYING && state !== states.BUFFERING) {
...
_defineProperty(Chromecast, "key", 'Chromecast');
