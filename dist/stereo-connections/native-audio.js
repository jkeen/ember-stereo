...
var _class, _descriptor, _descriptor2, _NativeAudio;
 ...
 * @return {Boolean}
 */
function durationGrowsWithTheClock(samples = []) {
...
let NativeAudio = (_class = (_NativeAudio = class NativeAudio extends Sound {
  constructor(...args) {
    ...
    _defineProperty(this, "_lastRecordedDurationMs", null);
    _initializerDefineProperty(this, "_durationHistory", _descriptor2, this);
    _defineProperty(this, "durationWorkaroundTask", buildTask(() => ({
      context: this,
      generator: function* () {
        ...
        if (macroCondition(isTesting())) {
          ...
          while (this.isPlaying) {
            ...
            if (Number.isFinite(duration)) {
              this._durationHistory = [...this._durationHistory.slice(-19), duration];
            }
            ...
            yield timeout(DURATION_SAMPLE_MS);
          }
        }
      }
    }), {
      restartable: true
    }, "durationWorkaroundTask", null));
    ...
    _defineProperty(this, "stopStreamAfterGraceTask", buildTask(() => ({
      context: this,
  ...
  _calculatePercentLoaded() {
    ...
    if (audio && audio.buffered && audio.buffered.length) {
      ...
      this.debug(`buffered ${Math.round(percentLoaded * 100)}% (${Math.round(total * 1000)}ms of ${Math.round(this._audioDuration())}ms)`);
      return {
        percentLoaded
      };
    } else {
  ...
  _audioDuration() {
    ...
    if (audio.duration * 1000 > IMPLAUSIBLE_DURATION_MS || this.probablyAStream) {
      ...
      let recorded = Number.isFinite(audio.duration) ? null : this._recordedDurationMs() ?? this._lastRecordedDurationMs;
...
export { NativeAudio as default, durationGrowsWithTheClock };
