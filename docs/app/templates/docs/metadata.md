...
Sometimes the media's own clock isn't the timeline a listener is moving through. A live stream has no duration at all, but if it's carrying a scheduled show, that show's window is a real timeline and the one your in-app scrubber probably draws.
...
Each action takes one of three answers:
...
`previoustrack` and `nexttrack` have no default behaviour, since a sound has no idea what the next track is. They appear only if you register them. Note that on iOS they take the place of the seek buttons rather than joining them.
