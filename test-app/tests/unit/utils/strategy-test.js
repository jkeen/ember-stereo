...
module('Unit | Utility | strategy', function (hooks) {
  ...
  test('strategy can be created', function (assert) {
    ...
    assert.strictEqual(strategy.key, 'NativeAudio');
    assert.strictEqual(strategy.name, 'Native Audio');
    assert.strictEqual(strategy.url, url.href);
    assert.strictEqual(strategy.mimeType, 'audio/mpeg');
  });
