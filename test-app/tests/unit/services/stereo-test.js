...
module('Unit | Service | stereo', function (hooks) {
  ...
  test('position gets polled regularly on the currentSound but not on the others', function (assert) {
    ...
    assert.strictEqual(
      spy1.callCount,
      0,
      'sound 1 should not have been polled yet'
    );
    assert.strictEqual(
      spy2.callCount,
      0,
    ...
    assert.strictEqual(
      spy1.callCount,
      4,
      'sound 1 should have been polled 4 times'
    );
    assert.strictEqual(
      spy2.callCount,
      0,
      'sound 2 should not have been polled yet'
    );
    ...
    assert.strictEqual(
      spy2.callCount,
      2,
      'sound 2 should have been polled twice'
    );
  ...
  test('toggleMute returns sound to previous level', function (assert) {
    ...
    assert.strictEqual(service.get('volume'), 0, 'volume should be zero');
  ...
  test('service has access to the current sound inside the play callback', async function (assert) {
    ...
    assert.strictEqual(sound.position, service.get('position'));
  });
  ...
  test('current-sound-interrupted event does not fire when position gets changed', async function (assert) {
    ...
    sound.position = 100;
    sound.position = 1500;
    assert.strictEqual(callCount, 0, 'interrupt should not have been called');
  });
